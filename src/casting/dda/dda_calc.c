...
static void	set_step_and_sidedist(t_dda *dda)
{
	...
	else
	{
		dda->step_map_x = 1;
		dda->side_dist_x = \
				(dda->map_pos.x + 1.0 - dda->player_pos.x) * dda->delta_dist_x;
	}
	if (dda->ray_dir.y < 0)
	{
		dda->step_map_y = -1;
	...
	else
	{
		dda->step_map_y = 1;
		dda->side_dist_y = \
				(dda->map_pos.y + 1.0 - dda->player_pos.y) * dda->delta_dist_y;
	}
}
...
void	draw_info_calc(t_dda *dda)
{
	...
	dda->draw_info.line_height = (1 / perp_wall_dist) * SCR_HEIGHT;
